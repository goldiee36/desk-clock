...
.Features software version v0.2
* button 1 short press put device in sleep (almost no power consumption)
* power off automatically if the battery reading is below 3.5V (power on with the left or the middle button)
* change screen with short press of the middle button (show temperature, humidity and battery voltage)
* on time is more then 100 hours
